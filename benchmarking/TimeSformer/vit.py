...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # Classifier head
        self.head = nn.ModuleList([nn.Linear(embed_dim, num_classes) for _ in range(20)])
    ...
    def forward(self, x):
        x = self.forward_features(x) # [B, 728]

        output = []
        for i in range(20):
            out = self.head[i](x) # [B, 5]
            output.append(out)
        x = torch.stack(output, dim=1) # [B, 20, 5]

...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        acc_cats = torch.mean((preds == targets).to(torch.float), dim=0)  # [num_cats, ]
    ...
    print('** Train Acc {top1.global_avg:.4f}'.format(top1=metric_logger.train_acc))
...
def validation_one_epoch(data_loader, model, device, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if ds:
            ...
            with amp_autocast:
                output = model(videos) # [B, 20, 5]
                output = torch.permute(output, (0, 2, 1)) # [B, 5, 20]
                loss = criterion(output, target)

        # acc1, acc5 = accuracy(output, target, topk=(1, maxk))
        output = torch.permute(output, (0, 2, 1)) # [B, 20, 5]
        preds = torch.argmax(output, dim=-1) # [B, 20]
        acc_cats = torch.mean((preds == target).to(torch.float), dim=0)  # [20, ]
        acc1 = torch.mean(acc_cats)
...
def final_test(data_loader, model, device, file, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    for i in range(num_cats):
        metric_logger.add_meter(f'test_acc_cat_{i}', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        acc_cats = torch.mean((preds == target).to(torch.float), dim=0)  # [num_cats, ]
        acc1 = torch.mean(acc_cats)

        for i in range(num_cats):
            metric_logger.update(**{f'test_acc_cat_{i}': acc_cats[i].item()})
    ...
    for i in range(num_cats):
        print('** Test Acc {cat} {top1.global_avg:.4f}'.format(cat=i, top1=metric_logger.meters[f'test_acc_cat_{i}']))

...
class VideoClsDataset(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        # Process the BASKET data file

        with PathManager.open(self.anno_path, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()[1:]):
                path, label_raw = path_label.split(',', 1)
    ...
    def loadvideo_decord(self, sample, sample_rate_scale=1, chunk_nb=0):
        ...
        try:
            ...
            all_index = []
            # Apply uniform sampling strategy

            for i in range(self.num_segment):

                index = np.linspace(0, seg_len - 1, num=self.clip_len) # uniform sampling

...
def train_class_batch(model, samples, target, criterion):
    ...
    outputs = torch.permute(outputs, (0, 2, 1)) # [B, 5, 20]
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(
        ...
        # print(f'train start {samples.shape} {targets.shape}')  # [B, 3, 16, 224, 224] [B, 20]
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16): # was bfloat16
                loss, output = train_class_batch(model, samples, targets,
                                                 criterion)
...
def final_test(data_loader, model, device, file):
    ...
    print(
        #'* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
        '* Acc@1 {top1.global_avg:.4f} loss {losses.global_avg:.3f}'
        .format(
            top1=metric_logger.acc1,
            #top5=metric_logger.acc5,
            losses=metric_logger.loss))
